...
function promisifyRequest(req) {
  return function(options) {
    // should call with one arg only; no cb.
    let args = [options];
    return new Promise ( (resolve, reject) => {
      args.push( (e, response, body) => (e) ? reject({error:e, response, body}) : resolve({response, body}) );
      req.apply(null, args);
    });
  };
}
...
function aliasify(s) {
  return removeDiacritics(s).replace(new RegExp(' ','g'), '-').toLowerCase();
}

function genCertOptions (certName){
  return {
